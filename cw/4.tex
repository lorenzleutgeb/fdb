...
Next, we assume that $Q'$ is not a subquery of $Q$ towards a contradiction. Since $Q'$ is not a subquery of $Q$, there is some relational atom $R(\bar{t}) \in L'$ such that $R(\bar{t}) \not \in L$. However, this contradicts the existence of $h$, $h'$ as follows: By definition of a query homomorphism, we ha ve $h(L') \subseteq L$, but $h(R(\bar{t}))$ cannot be in $L$.

Next, we show that $Q'$ is a subquery of $Q$: Since $h'$ exists, and $m < n$.

Since 

From there, we show that $Q'$ fulfills all conditions for $Q_0$.
\end{proof}
